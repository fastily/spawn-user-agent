...
class SpawnUserAgent:
    ...
    def firefox() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({platform}; rv:{version}.0) Gecko/20100101 Firefox/{version}.0" for platform in (f"{_MAC_PREFIX} 10.15", _WIN_PREFIX, _UBUNTU_PREFIX) for version in range(140, 147)]
    ...
    def chrome() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({platform}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{version}.0.0.0 Safari/537.36" for platform in (f"{_MAC_PREFIX} 10_15_7", _WIN_PREFIX, "X11; Linux x86_64") for version in range(137, 144)]
    ...
    def safari() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({_MAC_PREFIX} 10_15_7) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{major}.{minor} Safari/605.1.15" for major, minors in [(18, range(6, 8)), (26, range(0, 3))] for minor in minors]
    ...
    def safari_mobile() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{major}.{minor} Mobile/15E148 Safari/604.1" for major, minors in [(26, range(0, 3))] for minor in minors]
