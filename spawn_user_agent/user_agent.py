...
class SpawnUserAgent:
    ...
    def firefox() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({platform}; rv:{version}.0) Gecko/20100101 Firefox/{version}.0" for platform in (f"{_MAC_PREFIX} 10.15", _WIN_PREFIX, _UBUNTU_PREFIX) for version in range(129, 137)]
    ...
    def chrome() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({platform}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{version}.0.0.0 Safari/537.36" for platform in (f"{_MAC_PREFIX} 10_15_7", _WIN_PREFIX, "X11; Linux x86_64") for version in range(127, 135)]
    ...
    def safari() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} ({_MAC_PREFIX} 10_15_7) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{major}.{minor} Safari/605.1.15" for major, minors in [(17, range(5, 7)), (18, range(0, 4))] for minor in minors]
    ...
    def safari_mobile() -> list[str]:
        ...
        """
        return [f"{_GENERAL_TOKEN} (iPhone; CPU iPhone OS {major}_{minor} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/{major}.{minor} Mobile/15E148 Safari/604.1" for major, minors in [(17, range(6, 8)), (18, range(0, 4))] for minor in minors]
